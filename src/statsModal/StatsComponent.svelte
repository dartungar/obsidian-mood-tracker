...
<div class="chart-controls">
    <div class="range-presets-container">
        <button class="range-preset-btn" class:is-active={activePreset === 'week'} on:click={() => setRangePreset('week')}>{t("modals.stats.presets.week")}</button>
        <button class="range-preset-btn" class:is-active={activePreset === 'month'} on:click={() => setRangePreset('month')}>{t("modals.stats.presets.month")}</button>
        <button class="range-preset-btn" class:is-active={activePreset === 'year'} on:click={() => setRangePreset('year')}>{t("modals.stats.presets.year")}</button>
    </div>
    <div class="chart-type-container">
        <button class="chart-type-btn" class:is-active={chartType === 'bar'} on:click={() => chartType = 'bar'} aria-label={t("modals.stats.chartTypes.bar")}>{t("modals.stats.chartTypes.bar")}</button>
        <button class="chart-type-btn" class:is-active={chartType === 'line'} on:click={() => chartType = 'line'} aria-label={t("modals.stats.chartTypes.line")}>{t("modals.stats.chartTypes.line")}</button>
        <span class="chart-type-separator">|</span>
        <button class="chart-type-btn" class:is-active={chartType === 'doughnut'} on:click={() => chartType = 'doughnut'} aria-label={t("modals.stats.chartTypes.doughnut")}>{t("modals.stats.chartTypes.doughnut")}</button>
        <button class="chart-type-btn" class:is-active={chartType === 'polar'} on:click={() => chartType = 'polar'} aria-label={t("modals.stats.chartTypes.polar")}>{t("modals.stats.chartTypes.polar")}</button>
        <button class="chart-type-btn" class:is-active={chartType === 'radar'} on:click={() => chartType = 'radar'} aria-label={t("modals.stats.chartTypes.radar")}>{t("modals.stats.chartTypes.radar")}</button>
    </div>
</div>
<div class="date-picker-container">
    {t("modals.stats.from")}: <input bind:value={startDate} on:change={onManualDateChange} type="date" min="2000-01-01" pattern="\d{4}-\d{2}-\d{2}" />
    {t("modals.stats.to")}: <input bind:value={endDate} on:change={onManualDateChange} type="date" min="2000-01-01"  required pattern="\d{4}-\d{2}-\d{2}"/>
</div>

{#if isAggregate}
<div class="breakdown-container">
    {t("modals.stats.breakdown.label")}:
    <select bind:value={breakdownType}>
        <option value="mood-rating">{t("modals.stats.breakdown.moodRating")}</option>
        <option value="emotion">{t("modals.stats.breakdown.topEmotions")}</option>
        <option value="emotion-group">{t("modals.stats.breakdown.emotionGroup")}</option>
        <option value="day-of-week">{t("modals.stats.breakdown.dayOfWeek")}</option>
    </select>
</div>
{/if}
